...
module.exports = {
  ...
  async executeInteraction(interaction) {
    ...
    if (sub === 'egg') {
      ...
      if (baseLogger && baseLogger.sentry) { try { baseLogger.sentry.addBreadcrumb({ message: 'db.addEggs.start', category: 'db', data: { target: target.id, guildId, eggTypeId, amount } }); } catch (e) { try { require('../../utils/logger').get('command:give').warn('Failed to add sentry breadcrumb (db.addEggs.start)', { error: e && (e.stack || e) }); } catch (le) { try { fallbackLogger.warn('Failed logging give breadcrumb error (db.addEggs.start)', le && (le.stack || le)); } catch (ignored) { /* ignore */ } } } }
      await userModel.addEggsForGuild(target.id, guildId, amount, eggTypeId);
      if (baseLogger && baseLogger.sentry) { try { baseLogger.sentry.addBreadcrumb({ message: 'db.addEggs.finish', category: 'db', data: { target: target.id, guildId } }); } catch (e) { try { require('../../utils/logger').get('command:give').warn('Failed to add sentry breadcrumb (db.addEggs.finish)', { error: e && (e.stack || e) }); } catch (le) { try { fallbackLogger.warn('Failed logging give breadcrumb error (db.addEggs.finish)', le && (le.stack || le)); } catch (ignored) { /* ignore */ } } } }
      const safeReply = require('../../utils/safeReply');
