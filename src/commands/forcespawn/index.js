  ...
  async executeInteraction(interaction) {
    ...
    if (memberPerms && !memberPerms.has(PermissionsBitField.Flags.ManageGuild) && interaction.user.id !== interaction.guild.ownerId) {
      await safeReply(interaction, {
        ...buildNoticeV2Payload({ message: 'You need Manage Server permission to run this.', tone: 'permission' }),
        ephemeral: true
      }, { loggerName: 'command:forcespawn' });
      return;
    }
    ...
    try {
      ...
      if (spawned) {
        await safeReply(interaction, {
          ...buildNoticeV2Payload({
            title: '✅ Force Spawn Complete',
            message: `${emojis.egg || ''} Forced egg spawn triggered${eggTypeId ? `: ${eggTypeId}` : ''}!`,
            tone: 'info'
          }),
          ephemeral: true
        }, { loggerName: 'command:forcespawn' });
        ...
        await safeReply(interaction, {
          ...buildNoticeV2Payload({
            title: '⚠️ Spawn Skipped',
            message: 'No spawn occurred: a spawn recently happened. Try again in a few seconds.',
            tone: 'requirement'
          }),
          ephemeral: true
        }, { loggerName: 'command:forcespawn' });
      ...
      await safeReply(interaction, {
        ...buildNoticeV2Payload({
          message: `Failed to force spawn eggs: ${err && err.message ? err.message : 'Unknown error'}`,
          tone: 'error'
        }),
        ephemeral: true
      }, { loggerName: 'command:forcespawn' });
